...
class Weather:
    def __init__(self, locale):
        ...
        self.url = (
            "https://api.weather.gov/points/" + str(self.lat) + "," + str(self.lon)
        )

    def getWeather(self):
        forecast_url = json.loads(requests.get(self.url).content)["properties"][
            "forecastHourly"
        ]
        ...
        print(
            ...
            + str(res["properties"]["periods"][0]["temperature"])
            + "°F"
        )
        print("Windspeed: " + str(res["properties"]["periods"][0]["windSpeed"]))
        print(
            "Current conditions: "
            + str(res["properties"]["periods"][0]["shortForecast"])
        )
